...
*/
   ...
   hist_efficiency = new TH1F("efficiency","efficiency",50,-0.1,1.1);
...
//       std::cout<<"\n";
//       std::cout<< "n of b:\t" << isB.size() <<"\t";
       match=0;
       int bool_v[size_child];
       for(int k=0;k<size_child;k++)  bool_v[k]=0;
       ...
       hist_efficiency->Fill((float) match/size_child);
//       std::cout<<"\t"<< "n of matched tracks:\t"<< match << "\tn of child tracks(denominator):\t" << size_child << "\tratio:\t" <<(float) match/size_child<<"\t";
...
//         std::cout<<jet_trk_parent_pdgid[*it][i]<<"\n";
       }
